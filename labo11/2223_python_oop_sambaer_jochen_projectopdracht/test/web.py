...
def generate_names():
    num_names = int(request.args.get('num_names', 1000))  # Default to 10 names if not specified
...
def submit():
    ...
    try:
        ...
        bike_share_system.users = generate_random_users(input_int)
        bike_share_system.bikes = generate_random_bikes(input_int)
    except ValueError:
        return 'Invalid input!'
    logger.debug(input_int)
...
if __name__ == "__main__":
    ...
    bike_share_system.users = generate_random_users(10)
    bike_share_system.bikes = generate_random_bikes(10)
